...
Begin
    ...
    Set @Sql = N'Create or Alter View vMedicationList AS
                 Select ' + @Columns + N'
...
Begin
	...
	Set @RowCount = 
		(
		Select count(distinct M.PersonID)
...
Create or Alter Proc uspChangeStatus
@ID Int,
@Act TinyInt
as
	Begin
...
as
	Begin
		...
		Else
			Begin
				Print 'Warning!!! Diagnose already exists!';
			End;
	End;
Go
...
as
	Begin
		Set Nocount On;
		If not exists (Select 1 From Medical.PacientDiagnoses Where DiagnoseID  = @DID and PersonID = @PID)
			Begin
				Insert Into Medical.PacientDiagnoses
					(
					PersonID,
					DiagnoseID
					)
		...
		Else
			Begin
				Print 'Warning!!! Current patient already has the diagnosis set!';
			End;
	End;
Go
